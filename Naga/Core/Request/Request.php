 ...
 * @package Naga\Core\Request
 */
class Request extends nComponent
{
	// http methods
	const MethodGet = 1;
	const MethodPost = 2;
	...
	/**
	 * Echoes the request as a string.
	 */
	public function dumpRequest()
	{
		echo $this->requestAsString();
	}
	 ...
	 *
	 * @return string
	 * @throws \Naga\Core\Exception\RequestException
	 */
	...
	public function uri()
	{
		$uri = $this->fullUri();
		$queryPos = strpos($uri, '?');
		$uri = substr($uri, 0, $queryPos !== false ? $queryPos : strlen($uri));
		return $uri == '/' ? '' : $uri;
	}
	...
	/**
	 * Gets an array with the http methods in format array(constValue => string)
	 *
	 * @return array
	 */
	public function httpMethodList()
	{
		return array(
			self::MethodGet => 'get',
